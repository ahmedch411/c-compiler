...
void printElement(ElementPtr symelement) {
    if (symelement != NULL) {
    	if (symelement->stype->kind == 3) {
    		TypePtr tempType = (Type*) malloc(sizeof(Type));
    		tempType = symelement->stype;
    		printf("Function at Line %d: %s", symelement->linenumber,symelement->id);
    		while(tempType)
    		{
    			printf(" %s", typeNameConv(tempType));
    			tempType = tempType->function;
    		}
    		printf("\n");        
    	}
    	else {
...
char* typeNameConv (TypePtr typeGiven) {
	switch (typeGiven->kind) {
		case 0: return "VOID";
		case 1: return "INT";
		case 2: return "ARRAY";
